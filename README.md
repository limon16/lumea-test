...
- Секція «How it works»: 4 картки кроків зі scroll-накладанням
  (CSS `position: sticky` + GSAP ScrollTrigger), панель товарів праворуч (десктоп) і overlay-шторка знизу
  (мобільний).
...
- Модель товарів/категорій/міток/announcement bar у Strapi 5 (PostgreSQL),
  назви та описи CMS-полів — українською.
...
Hero використовує локальні JPG, фонові ефекти й чотири кроки — SVG,
Open Graph — `og-image.png` у `apps/web/public/`. Відповідність цих assets
фінальному макету потрібно перевіряти за Figma.

Зображення товарів надходять зі Strapi. Якщо зображення не задане,
картка показує плейсхолдер. Для production-домену CMS необхідно
задати `NEXT_PUBLIC_STRAPI_URL` і, за потреби, `IMAGE_ORIGINS`;
`apps/web/next.config.ts` формує `images.remotePatterns` автоматично.
...
`apps/cms/src/validation/product.ts` (через middleware у `apps/cms/src/index.ts`) не дасть
...
обрано `byVariation`, а middleware Document Service перевіряє повний запис, включно з частковими оновленнями та вкладеними варіантами.
...
власних цін → розрахована ціна £23.80 відображається як **£24** (цілі фунти).
...
- `Normal` → `50 ml`: `priceOverride: 32`, `discountPercent: 10`,
  `stock: 3` → **£29** (розрахована ціна £28.80), «Only 3 left».
...
4. Environment Variables (Production):
   ...
   - `IMAGE_ORIGINS` — додаткові дозволені origins сховища зображень через кому, якщо фото не на домені CMS.
   - `STRAPI_URL` — опційна внутрішня адреса CMS для серверних запитів.
   - `NEXT_PUBLIC_SITE_URL` — фінальний домен на Vercel
     (напр. `https://lumea.vercel.app` або кастомний домен).

`NEXT_PUBLIC_SITE_URL` використовується для canonical та Open Graph. У Vercel
задай повний URL із `https://` у **Settings → Environment Variables** для Production
і зроби новий deployment. Не копіюй локальне значення `http://localhost:3000`.
Якщо змінну не задано, код використовує системний `VERCEL_PROJECT_PRODUCTION_URL`,
далі `VERCEL_URL`; localhost залишається резервом для локального запуску.
...
- Початкова SSR-відповідь містить перші 12 категорій і перші 12 товарів
  початкової категорії. Наступні сторінки підвантажуються біля кінця
  відповідного горизонтального списку через `/api/catalog`.
- API приймає `kind=products|categories`, `page`, а для товарів — `category`
  і `search`. Фільтрація товарів відбувається у Strapi до пагінації.
- Desktop і mobile ділять кеш сторінок категорій; подвійні паралельні
  запити блокуються, відповіді попередньої категорії не заміняють активну.
  При помилці є кнопка повтору, при порожній категорії — empty state.
- Hero CTA прокручує до How it works. Header відкриває меню, пошук,
  wishlist і кошик. Деталі товару відкриваються в діалозі.
- Wishlist і кошик — локальні demo-стани поточної сторінки: після reload
  скидаються, серверних замовлень чи оплати немає. Кошик розрізняє обрані
  варіанти товару, дозволяє змінювати кількість і видаляти рядки.
- Кінцева ціна, задана в CMS, не відновлюється з округленого відсотка.
  Ціни на екрані округлюються до найближчого цілого фунта (`Math.round`),
  без копійок. Сума кошика складається з показаних цілих цін за одиницю.
- Висота Product Card не має верхнього обмеження; довгі назви та групи
  варіацій розширюють її. Висота sticky-секції вимірює праву панель.
- Для reduced motion картки кроків залишаються в потоці документа.
  Радіогрупи підтримують стрілки/Home/End; діалоги — Escape і фокус.

### Vercel і CMS

Vercel використовується для `apps/web`. Для Strapi потрібен окремий
Node.js-сервіс із PostgreSQL (наприклад, вже описаний вище backend).
`NEXT_PUBLIC_STRAPI_URL` — публічна адреса цього сервісу; `IMAGE_ORIGINS`
додає домен зовнішнього media storage. Зміна цих build-time параметрів
потребує нового деплою frontend.

Поточний upload provider Strapi зберігає файли локально. Для production
підключіть постійний диск до `apps/cms/public/uploads` або налаштуйте
зовнішній upload provider. Тимчасовий диск хостингу не є постійним
сховищем фото. Конкретний provider, backend-домен, production-БД і
Admin-акаунт у межах цих змін не створювались.

Після реалізації цих змін тести, лінтер, typecheck, збірка й браузерні
перевірки не запускались за домовленістю із замовником. Наявність файлів
тестів не означає, що поточні зміни пройшли перевірку.
